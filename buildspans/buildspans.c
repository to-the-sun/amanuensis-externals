...
void buildspans_save_current_state(t_buildspans *x);
...
void buildspans_visualize_memory(t_buildspans *x) {
    ...
    for (long p = 0; p < unique_palette_count; p++) {
        ...
        // Palette header
        double pal_current_offset = 0.0;
        long pal_bar_length = buildspans_get_bar_length(x); // Default to current global if not found
...
void *buildspans_new(t_symbol *s, long argc, t_atom *argv) {
    ...
    if (x) {
        ...
        x->palette_states = dictionary_new();
        x->current_track = 0;
...
void buildspans_save_current_state(t_buildspans *x) {
    if (x->current_palette == NULL || x->current_palette == _sym_nothing) return;

    t_dictionary *d = NULL;
    if (dictionary_getdictionary(x->palette_states, x->current_palette, (t_object **)&d) != MAX_ERR_NONE) {
        d = dictionary_new();
        dictionary_appenddictionary(x->palette_states, x->current_palette, (t_object *)d);
    }
...
void buildspans_anything(t_buildspans *x, t_symbol *s, long argc, t_atom *argv) {
    ...
    if (inlet_num == 3) {
        ...
        if (argc == 0) {
            buildspans_save_current_state(x);
            buildspans_load_state_for_palette(x, s);
