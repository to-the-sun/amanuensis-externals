...
typedef struct _rebar {
    ...
    void *out_done;      // Finished Bang (Index 0)
    void *out_log;       // Consolidated logging (Index 1)
...
void *rebar_intercept_outlet_new(void *x, const char *classname) {
    ...
    if (!rebar) return (outlet_new)((t_object *)x, (char *)classname);
    if (x == (void *)rebar) return (outlet_new)((t_object *)x, (char *)classname);
...
void rebar_intercept_outlet_anything(void *o, t_symbol *s, short ac, t_atom *av) {
    ...
    if (vo->type == MOD_NOTIFY) {
        ...
        else if (vo->index == 4) (outlet_anything)(x->out_log, s, ac, av);
    } else if (vo->type == MOD_BUILDSPANS) {
        if (vo->index == 2) rebar_crucible_anything(x->crucible_inst, s, (long)ac, av);
        else if (vo->index == 3) (outlet_anything)(x->out_log, s, ac, av);
    } else if (vo->type == MOD_CRUCIBLE) {
        if (vo->index == 3) (outlet_anything)(x->out_log, s, ac, av);
    }
}
...
void rebar_intercept_outlet_bang(void *o) {
    ...
    if (vo->type == MOD_NOTIFY) {
        if (vo->index == 0) {
            // Notify dump complete. Flush spans.
            rebar_buildspans_bang(x->buildspans_inst);

            // Pipeline finished (potentially across threads). Request final commit.
...
void *rebar_new(t_symbol *s, long argc, t_atom *argv) {
    ...
    if (x) {
        ...
        if (argc > 0 && atom_gettype(argv) == A_SYM && strncmp(atom_getsym(argv)->s_name, "@", 1) != 0) {
            ...
            // Dictionary is NOT released here; it's held by the object.
        }
        ...
        x->out_done = (bangout)((t_object *)x);

        critical_enter(g_rebar_crit);
        g_instantiating_rebar = x;

        // Force modules to have log enabled at instantiation for stable indices
        t_atom args[3];
...
void rebar_free(t_rebar *x) {
    ...
    if (d) {
        dictobj_unregister(d);
        object_release((t_object *)d); // Release retain
        object_release((t_object *)d); // Release original
    }
}

void rebar_do_copy_back(t_rebar *x) {
    common_log(x->out_log, x->log, "rebar", "Copying from temporary dictionary...");
    ...
    common_log(x->out_log, x->log, "rebar", "Isolated coordinated dump and comparison complete. Elapsed time: %.2f ms", systimer_gettime() - x->start_time);
    (outlet_bang)(x->out_done);
}

void rebar_int(t_rebar *x, long n) {
    x->start_time = systimer_gettime();
    common_log(x->out_log, x->log, "rebar", "Copying to temporary dictionary...");
...
void rebar_assist(t_rebar *x, void *b, long m, long a, char *s) {
    if (m == ASSIST_INLET) sprintf(s, "Inlet 1: (int) Trigger isolated coordinated dump with specified bar length.");
        ...
        switch (a) {
            case 0: sprintf(s, "Outlet 1: Finished bang when dictionary copy-back is complete"); break;
            case 1: sprintf(s, "Outlet 2: Consolidated Logging and Status messages"); break;
        }
    }
}
