...
#define buildspans_get_track_sym rebar_buildspans_get_track_sym
#define buildspans_get_bar_sym rebar_buildspans_get_bar_sym
#define buildspans_get_bar_atom rebar_buildspans_get_bar_atom
#define buildspans_set_bar_atom rebar_buildspans_set_bar_atom
#define buildspans_get_bar_atomarray rebar_buildspans_get_bar_atomarray
#define buildspans_set_bar_atomarray rebar_buildspans_set_bar_atomarray
#define buildspans_delete_bar_property rebar_buildspans_delete_bar_property
#define buildspans_has_bar_property rebar_buildspans_has_bar_property
...
typedef void *(*t_outlet_anything_fn)(t_outlet *o, t_symbol *s, short ac, t_atom *av);
typedef void *(*t_outlet_list_fn)(t_outlet *o, t_symbol *s, short ac, t_atom *av);
typedef void *(*t_outlet_int_fn)(t_outlet *o, t_atom_long n);
...
static t_outlet_anything_fn sdk_outlet_anything = NULL;
static t_outlet_list_fn sdk_outlet_list = NULL;
...
static int count_outlets(void *owner) {
    int count = 0;
    critical_enter(g_rebar_crit);
    t_outlet_node *curr = g_outlet_registry;
    while (curr) {
        if (curr->vo.owner == owner) count++;
        curr = curr->next;
    }
    critical_exit(g_rebar_crit);
    return count;
...
void rebar_intercept_outlet_int(void *o, t_atom_long n) {
    t_virt_outlet *vo = get_virt_outlet(o);
    if (!vo) { if (sdk_outlet_int) sdk_outlet_int(o, n); return; }
    t_rebar *x = get_rebar(vo->owner);
    if (!x) return;
...
void rebar_intercept_outlet_float(void *o, double f) {
    t_virt_outlet *vo = get_virt_outlet(o);
    if (!vo) { if (sdk_outlet_float) sdk_outlet_float(o, f); return; }
    t_rebar *x = get_rebar(vo->owner);
    if (!x) return;
...
void rebar_intercept_outlet_bang(void *o) {
    t_virt_outlet *vo = get_virt_outlet(o);
    if (!vo) { if (sdk_outlet_bang) sdk_outlet_bang(o); return; }
    t_rebar *x = get_rebar(vo->owner);
    if (!x) return;
...
void *rebar_new(t_symbol *s, long argc, t_atom *argv) {
    ...
    if (x) {
        ...
        if (argc > 0 && atom_gettype(argv) == A_SYM && strncmp(atom_getsym(argv)->s_name, "@", 1) != 0) {
            ...
            char tmp_name[256];
            snprintf(tmp_name, 256, "rebar_tmp_%p", x);
