...
t_dictionary *rebar_dictionary_deep_copy(t_dictionary *src);
...
void *rebar_new(t_symbol *s, long argc, t_atom *argv) {
    ...
    if (x) {
        ...
        x->out_done = sdk_bangout((t_object *)x);
...
void rebar_do_copy_back(t_rebar *x) {
    rebar_log(x, "Starting copy back from temporary dictionary");
    ...
    if (user_dict && tmp_dict) {
        ...
        t_dictionary *isolated = rebar_dictionary_deep_copy(tmp_dict);
        if (isolated) {
            dictionary_copyentries(isolated, user_dict, NULL);
            object_release((t_object *)isolated);
        }
    }
    ...
    double elapsed = 0;
    clock_getftime(&elapsed);
    elapsed -= x->start_time;
    rebar_log(x, "Function took %.2f ms", elapsed);
...
void rebar_int(t_rebar *x, long n) {
    clock_getftime(&x->start_time);
    rebar_log(x, "Starting copy to temporary dictionary");
    t_dictionary *user_dict = dictobj_findregistered_retain(x->user_dict_name);
    t_dictionary *tmp_dict = dictobj_findregistered_retain(x->tmp_dict_name);
    if (user_dict && tmp_dict) {
        dictionary_clear(tmp_dict);
        t_dictionary *isolated = rebar_dictionary_deep_copy(user_dict);
        if (isolated) {
            dictionary_copyentries(isolated, tmp_dict, NULL);
            object_release((t_object *)isolated);
        }
    }
    if (user_dict) object_release((t_object *)user_dict);
...
void rebar_assist(t_rebar *x, void *b, long m, long a, char *s) {
    if (m == ASSIST_INLET) sprintf(s, "Inlet 1: (int) Trigger isolated coordinated dump with specified bar length.");
        ...
        switch (a) {
            ...
            case 3: sprintf(s, "Outlet 4: Completion bang"); break;
