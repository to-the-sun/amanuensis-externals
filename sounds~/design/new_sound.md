...
- **Mandatory First Step:** Update the `SOUND_DESIGN_VERSION` macro in `sound_design.h` to the next increment (find the highest numbered folder in `sounds/` and add 1) **BEFORE** compiling or running `./audio_engine` for the first time. This prevents `./audio_engine` from copying or overwriting files in the previous version's folder.
