...
def extract_timestamp_from_filename(filename: str) -> Optional[str]:
    """Extract whatever is inside the square brackets as the timestamp/token."""
...
def update_timestamp_in_filename(filename: str, new_timestamp: str) -> str:
    ...
    if '[' in filename and ']' in filename:
        ...
        new_filename = re.sub(r'\[\d{4}-\d{1,2}-\d{1,2}\s\d{6}\]', f'[{new_timestamp}]', filename)
        if new_filename != filename:
            return new_filename
...
def main() -> None:
    ...
    if args.files:
        ...
        # 1. Scan for the timestamp of the files being incremented (numbers 1, 2, 3, 4) BEFORE renaming
        audio_files = list_audio_files(script_dir)
        numbered = collect_numbered_audio(audio_files)
        found_timestamp = None
        for target in numbered:
            if target.number in (1, 2, 3, 4):
                ts = extract_timestamp_from_filename(target.original_path.name)
                if ts:
                    found_timestamp = ts
                    break

        # 2. Look three levels above its current directory for transcript.json and move/rename it
        target_dir = script_dir.parent.parent.parent
        ...
        if transcript_path.is_file():
            ...
            if found_timestamp:
                dest_name = f"transcript [{found_timestamp}].json"
            else:
                # Fallback to current timestamp if no timestamp was found in the audio files
                current_ts = format_timestamp_token(datetime.now())
                dest_name = f"transcript [{current_ts}].json"

            dest_path = backup_dir / dest_name
            shutil.move(str(transcript_path), str(dest_path))
            print(f"Moved {transcript_path.name} to {dest_path}")

        # 3. Rename existing audio files in script_dir
