...
3. **Contiguous Span Padding**: Any missing multiples of the new `bar_length` from the lowest converted bar timestamp to the **old** highest plus the **old** `bar_length` (exclusive of that limiting bound itself) are filled in and added to the array in low to high order.
